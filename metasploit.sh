...
msfvar=4.16.16
msfpath='/data/data/io.neoterm/files/home'
if [ -d "$msfpath/metasploit-framework" ]; then
	echo "metasploit is installed"
	exit 1
fi
apt update
apt install -y autoconf bison clang coreutils curl findutils git apr apr-util libffi-dev libgmp-dev libpcap-dev postgresql-dev readline-dev libsqlite-dev openssl-dev libtool libxml2-dev libxslt-dev ncurses-dev pkg-config postgresql-contrib wget make ruby-dev libgrpc-dev ncurses-utils ncurses unzip zip tar postgresql termux-elf-cleaner
...
ln -s $msfpath/metasploit-framework/msfconsole /data/data/io.neoterm/files/usr/bin/
ln -s $msfpath/metasploit-framework/msfvenom /data/data/io.neoterm/files/usr/bin/

termux-elf-cleaner /data/data/io.neoterm/files/usr/lib/ruby/gems/2.4.0/gems/pg-0.20.0/lib/pg_ext.so
